import time, re, ccxt, json, os, gc, threading, urllib.request, urllib.error, urllib.parse
...
# ====================== COINMARKETCAP CHAIN FALLBACK ======================
#
# Binance, Bybit, MEXC and BingX don't expose withdrawal/deposit network data
# via ccxt without API keys. CoinMarketCap's /v1/cryptocurrency/info endpoint
# returns a `platform` field for every coin listing showing which blockchain
# it lives on. Since one symbol (e.g. USDT) has multiple CMC listings — one
# per chain — batching all listings for a symbol gives us its full chain set.
#
# API key is read from the CMC_API_KEY environment variable (set in Render
# dashboard). Never hardcoded.
#
# FLOW:
#   prefetch_networks_bulk(coins) is called once after Phase 1:
#     - Sends /v1/cryptocurrency/info?symbol=X,Y,Z in batches of 100
#     - Populates _cmc_cache: symbol → {chain: {withdraw:T, deposit:T}}
#   Phase 2 calls get_external_networks(coin) → pure dict lookup, zero HTTP.
#
# CREDIT COST:
#   1 credit per batch call. ~1808 coins / 100 per batch = ~19 credits/scan.
#   Free plan: 10 000 credits/month → comfortable for frequent scanning.

# Maps CMC platform name → our canonical chain name
CMC_PLATFORM_MAP = {
    "ethereum":                      "ETH",
...
# Native coins that have no CMC platform entry (platform is null) because they
# ARE the chain. We infer their chain directly from the symbol.
CMC_NATIVE_CHAIN = {
    "BTC":   "BTC",   "ETH":  "ETH",  "BNB":  "BSC",
    "SOL":   "SOL",   "TRX":  "TRON", "MATIC": "POLYGON",
    "AVAX":  "AVAX",  "TON":  "TON",  "ADA":  "ADA",
    "DOT":   "DOT",   "XRP":  "XRP",  "NEAR": "NEAR",
    "ALGO":  "ALGO",  "XLM":  "XLM",  "ATOM": "ATOM",
    "FTM":   "FTM",   "CRO":  "CRO",  "HBAR": "HBAR",
    "EOS":   "EOS",   "SUI":  "SUI",  "APT":  "APTOS",
}

_cmc_lock      = threading.Lock()
_cmc_cache     = {}      # symbol.upper() → {chain: {"withdraw": True, "deposit": True}}
_cmc_cache_ts  = {}      # symbol.upper() → epoch_sec of last fetch
_CMC_CACHE_TTL = 86400   # 24 h
_CMC_BASE      = "https://pro-api.coinmarketcap.com"
_CMC_BATCH     = 100     # max symbols per /info request (CMC limit)

def _get_cmc_api_key() -> str:
    return os.environ.get("CMC_API_KEY", "")

def _cmc_get(path: str, params: dict) -> dict | None:
    """
    HTTP GET to CoinMarketCap with API key header.
    Returns parsed JSON `data` field or None on any error.
    """
...
def _parse_cmc_info_response(data: dict) -> dict:
    """
    Convert a /v1/cryptocurrency/info response `data` dict into our
    internal format: { SYMBOL: { chain: {withdraw:T, deposit:T} } }

    CMC returns data[symbol] as either a single object or a list of objects
    (one per chain deployment). We union all chains for the same symbol.
    """
    result = {}
    for sym, entries in data.items():
        sym_upper = sym.upper()
        # CMC may return a dict (single) or list (multi-chain symbol)
        ...
        for entry in entries:
            ...
            if platform:
                # Token deployed on another chain
                pname     = (platform.get("name") or "").lower().strip()
                canonical = CMC_PLATFORM_MAP.get(pname)
                if not canonical:
                    canonical = normalize_chain(pname.replace(" ", "_").upper())
                if canonical:
                    chains[canonical] = {"withdraw": True, "deposit": True}
            else:
                # Native coin — infer chain from symbol
                native = CMC_NATIVE_CHAIN.get(sym_upper)
                if native:
                    chains[native] = {"withdraw": True, "deposit": True}
        if chains:
            result[sym_upper] = chains
    return result


def get_external_networks(coin: str) -> dict:
    """
    Return cached chain map for `coin`.
    Must be called after prefetch_networks_bulk() so the cache is warm.
    Returns {} if CMC had no data for this coin.
    """
    with _cmc_lock:
        return _cmc_cache.get(coin.upper(), {})


def prefetch_networks_bulk(coins_iterable, logger=None):
    """
    Pre-populate _cmc_cache for every coin in coins_iterable before Phase 2.

    Sends /v1/cryptocurrency/info in batches of _CMC_BATCH symbols.
    Each batch costs 1 API credit. No per-coin rate-limit delay needed
    since we're doing bulk calls.
    """
    if not _get_cmc_api_key():
        if logger:
            logger("CMC: CMC_API_KEY env var not set — chain validation unavailable")
        return

    coins = list(dict.fromkeys(c.upper() for c in coins_iterable))

    # Filter out coins already freshly cached
    now       = time.time()
    to_fetch  = []
    with _cmc_lock:
        for coin in coins:
            ts = _cmc_cache_ts.get(coin, 0)
            if coin not in _cmc_cache or (now - ts) >= _CMC_CACHE_TTL:
                to_fetch.append(coin)

    if not to_fetch:
        if logger:
            logger(f"CMC: all {len(coins)} coins already cached")
        return

    batches    = [to_fetch[i:i + _CMC_BATCH] for i in range(0, len(to_fetch), _CMC_BATCH)]
    total_ok   = 0
    total_miss = 0

    if logger:
        logger(f"CMC: fetching {len(to_fetch)} coins in {len(batches)} batch(es) …")

    for batch in batches:
        data = _cmc_get("/v1/cryptocurrency/info", {
            "symbol":  ",".join(batch),
            "aux":     "platform",   # only request the platform field to minimise payload
        })
        if not data:
            total_miss += len(batch)
            continue

        parsed = _parse_cmc_info_response(data)
        ts_now = time.time()
        with _cmc_lock:
            for coin in batch:
                networks = parsed.get(coin, {})
                _cmc_cache[coin]    = networks
                _cmc_cache_ts[coin] = ts_now
                if networks:
                    total_ok += 1
                else:
                    total_miss += 1
    ...
    if logger:
        logger(f"CMC: prefetch done — {total_ok} coins resolved, "
               f"{total_miss} not found / no chain data")
...
def choose_common_chain(b_slim_curr, s_slim_curr, coin, exclude_chains, include_all):
    try:
        ...
        # Fall back to CoinMarketCap data for exchanges
...
def run_scan(settings, logger):
    ...
    # the CMC cache so Phase 2 never makes live HTTP calls.
