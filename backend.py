import time, re, ccxt, json, os, gc
...
# ====================== CORE SCAN (Sequential & Memory Optimized) ======================
def run_scan(settings, logger):
    ...
    logger("🚀 Starting scan (Sequential Mode)")
    ...
    MAX_PAIRS_PER_COMBO = 200

    # Load all exchanges first
    ex_objs = {}
    for ex_id in set(buy + sell):
        try:
            ...
            ex.load_markets()
            ex_objs[ex_id] = ex
            logger(f"✓ Loaded markets → {EXCHANGE_NAMES.get(ex_id, ex_id)} ({len(ex.markets)} pairs)")
        except Exception as e:
            logger(f"⚠️ Skipped {EXCHANGE_NAMES.get(ex_id, ex_id)} (load failed: {str(e)[:50]})")
            continue

    if not ex_objs:
        logger("❌ No exchanges loaded")
        return []

    # Fetch tickers for all exchanges
    bulk = {}
    for ex_id, ex in ex_objs.items():
        try:
            bulk[ex_id] = fetch_tickers_safe(ex, EXCHANGE_NAMES.get(ex_id, ex_id))
            tick_count = len(bulk[ex_id]) if bulk[ex_id] else 0
            logger(f"✓ Fetched tickers → {EXCHANGE_NAMES.get(ex_id, ex_id)} ({tick_count} tickers)")
        except Exception as e:
            logger(f"⚠️ Skipped tickers for {EXCHANGE_NAMES.get(ex_id, ex_id)}: {str(e)[:50]}")
            continue

    results = []
    current_keys = []
    total_combos = len(buy) * len(sell)
    current_combo = 0
    ...
    for b_id in buy:
        for s_id in sell:
            ...
            if b_id == s_id: 
                continue
            if b_id not in ex_objs or s_id not in ex_objs: 
                continue

            logger(f"🔍 Processing combo {current_combo}/{total_combos}: {b_id} → {s_id}")
            ...
            symbols = symbols[:MAX_PAIRS_PER_COMBO]
            ...
            # Add combo results to main results
            results.extend(combo_results)
            logger(f"✓ Combo {b_id}→{s_id}: {len(combo_results)} opportunities")

            # Force garbage collection between combos
            gc.collect()
    ...
    logger(f"✅ Scan finished — {len(results)} total opportunities")
    return results
