import time, re, ccxt, json, os, gc, threading
from concurrent.futures import ThreadPoolExecutor, as_completed
from flask import Flask, request, jsonify, render_template_string
from flask_cors import CORS

app = Flask(__name__)
CORS(app)

# ====================== SETTINGS ======================
SETTINGS_FILE = "settings.json"

def load_settings():
    if os.path.exists(SETTINGS_FILE):
        try:
            with open(SETTINGS_FILE) as f:
                return json.load(f)
        except:
            pass
    return {}

def save_settings(settings):
    try:
        with open(SETTINGS_FILE, "w") as f:
            json.dump(settings, f, indent=2)
    except:
        pass

saved = load_settings()

# ====================== CONSTANTS ======================
TOP_EXCHANGES = [
    "binance", "okx", "coinbase", "kraken", "bybit", "kucoin",
    "mexc", "bitfinex", "bitget", "gateio", "crypto_com",
    "upbit", "whitebit", "poloniex", "bingx", "lbank",
    "bitstamp", "gemini", "bitrue", "xt", "huobi", "bitmart", "coinex"
]

EXCHANGE_NAMES = {
    "binance": "Binance", "okx": "OKX", "coinbase": "Coinbase",
    "kraken": "Kraken", "bybit": "Bybit", "kucoin": "KuCoin",
    "mexc": "MEXC", "bitfinex": "Bitfinex", "bitget": "Bitget",
    "gateio": "Gate.io", "crypto_com": "Crypto.com", "upbit": "Upbit",
    "whitebit": "WhiteBIT", "poloniex": "Poloniex", "bingx": "BingX",
    "lbank": "LBank", "bitstamp": "Bitstamp", "gemini": "Gemini",
    "bitrue": "Bitrue", "xt": "XT", "huobi": "Huobi", "bitmart": "BitMart", "coinex": "CoinEx"
}

# FIX #3: Added "binance" — was missing, caused fetch_tickers() to silently
# fail with ArgumentsRequired in CCXT v4+, returning {} for all Binance tickers
EXTRA_OPTS = {
    "binance":  {"options": {"defaultType": "spot"}},
    "bybit":    {"options": {"defaultType": "spot"}},
    "okx":      {"options": {"defaultType": "spot"}},
    "bingx":    {"options": {"defaultType": "spot"}},
    "mexc":     {"options": {"defaultType": "spot"}},
    "bitrue":   {"options": {"defaultType": "spot"}},
    "xt":       {"options": {"defaultType": "spot"}},
    "huobi":    {"options": {"defaultType": "spot"}},
    "bitmart":  {"options": {"defaultType": "spot"}},
    "coinex":   {"options": {"defaultType": "spot"}},
}

USD_QUOTES = {"USDT", "USD", "USDC", "BUSD"}
LOW_FEE_CHAIN_PRIORITY = ["TRC20", "BSC", "SOL", "MATIC", "ARB", "OP", "TON", "AVAX", "ETH"]

LEV_REGEX = re.compile(r"\b(\d+[LS]|UP|DOWN|BULL|BEAR)\b", re.IGNORECASE)

# FIX #1: Replaced the old CHAIN_ALIASES that swapped names with each other
# (BSC→BEP20, BEP20→BSC meant they never equaled each other after normalization).
# Now every alias maps to one single canonical name so intersection works correctly:
#   normalize_chain("BSC")   → "BSC"
#   normalize_chain("BEP20") → "BSC"   ← same canonical, they now match ✅
#   normalize_chain("MATIC") → "POLYGON"
#   normalize_chain("Polygon") → "POLYGON" ← same canonical, they now match ✅
CHAIN_CANONICAL = {
    "BEP20":    "BSC",
    "BSC":      "BSC",
    "MATIC":    "POLYGON",
    "POLYGON":  "POLYGON",
    "OP":       "OPTIMISM",
    "OPTIMISM": "OPTIMISM",
    "ARB":      "ARBITRUM",
    "ARBITRUM": "ARBITRUM",
    "TRC20":    "TRON",
    "TRON":     "TRON",
}

def normalize_chain(name: str) -> str:
    n = name.upper().strip()
    return CHAIN_CANONICAL.get(n, n)

# ====================== THREAD-SAFE RUNTIME STATE ======================
# FIX #7: All shared globals now protected by a lock.
# Without this, concurrent Flask requests (gunicorn threaded / multiple scans)
# would simultaneously read and write op_cache / lifetime_history / last_seen_keys,
# causing corrupted stability data and potential KeyError crashes.
_cache_lock = threading.Lock()
op_cache = {}
lifetime_history = {}
last_seen_keys = set()

# ====================== HELPERS ======================
def parse_symbol(symbol: str):
    try:
        base, quote = symbol.split("/")[0], symbol.split("/")[1].split(":")[0]
        return base, quote
    except:
        return None, None

def market_price_from_ticker(t):
    if not t: return None
    last = t.get("last")
    if last is not None:
        try:
            v = float(last)
            if v > 0: return v
        except:
            pass
    bid, ask = t.get("bid"), t.get("ask")
    if bid is not None and ask is not None:
        try: return (float(bid) + float(ask)) / 2.0
        except: return None
    return None

# FIX #4: Reduced from 300s (5 minutes) to 30s.
# 5-minute-old prices are dangerous for arbitrage — the spread could have
# completely reversed. 30s is still generous but far safer.
def is_ticker_fresh(t, max_age_sec=30):
    ts = t.get("timestamp")
    if ts is None: return True
    return (int(time.time() * 1000) - int(ts)) <= max_age_sec * 1000

def fmt_usd(x):
    try:
        x = float(x or 0)
        if x < 0: return "$0"    # FIX #9: clamp negative volumes to $0
        if x >= 1e9: return f"${x/1e9:.2f}B"
        if x >= 1e6: return f"${x/1e6:.2f}M"
        if x >= 1e3: return f"${x/1e3:.0f}K"
        return f"${x:,.0f}"
    except: return "$0"

def secs_to_label(secs):
    return f"{int(secs)}s" if secs < 90 else f"{secs/60:.1f}m"

def update_lifetime_for_disappeared(current_keys):
    # FIX #7: Lock before touching shared globals
    with _cache_lock:
        gone = last_seen_keys - set(current_keys)
        for key in gone:
            trail = op_cache.get(key, [])
            if trail:
                duration = trail[-1][0] - trail[0][0]
                if duration > 0:
                    hist = lifetime_history.setdefault(key, [])
                    hist.append(duration)
                    # FIX #5: Cap history per key at 50 entries.
                    # Without this, every disappeared opportunity appends forever —
                    # a guaranteed memory leak on a long-running server.
                    if len(hist) > 50:
                        lifetime_history[key] = hist[-50:]
        last_seen_keys.clear()
        last_seen_keys.update(current_keys)

def stability_and_expiry(key, current_profit):
    now = time.time()
    # FIX #7: Lock before touching shared globals
    with _cache_lock:
        trail = op_cache.get(key, [])
        if not trail:
            op_cache[key] = [(now, current_profit)]
            return "⏳ new", "~unknown"
        trail.append((now, current_profit))
        op_cache[key] = trail[-30:]
        duration = trail[-1][0] - trail[0][0]
        observed = f"⏳ {secs_to_label(duration)} observed"
        hist = lifetime_history.get(key, [])
        if not hist:
            expiry = "~unknown"
        else:
            avg = sum(hist) / len(hist)
            remaining = avg - duration
            expiry = "⚠️ past avg" if remaining <= 0 else f"~{secs_to_label(remaining)} left"
        return observed, expiry

INFO_VOLUME_CANDIDATES = [
    "quoteVolume", "baseVolume", "vol", "vol24h", "volCcy24h", "volValue",
    "turnover", "turnover24h", "quoteVolume24h", "amount", "value",
    "acc_trade_price_24h", "quote_volume_24h", "base_volume_24h",
]

def safe_usd_volume(ex_id, symbol, ticker, price, all_tickers):
    try:
        base, quote = parse_symbol(symbol)
        if not base or not quote: return 0.0
        q_upper = quote.upper()
        qvol = ticker.get("quoteVolume")
        bvol = ticker.get("baseVolume")
        if q_upper in USD_QUOTES and qvol:
            return float(qvol)
        if bvol and price:
            return float(bvol) * float(price)

        info = ticker.get("info") or {}
        raw = None
        for k in INFO_VOLUME_CANDIDATES:
            val = info.get(k)
            if val is not None:
                try:
                    fval = float(val)
                    if fval > 0:
                        raw = fval
                        break
                except: continue
        if raw is not None:
            if q_upper in USD_QUOTES:
                return float(raw)
            for conv in ["USDT", "USDC", "USD"]:
                conv_sym = f"{q_upper}/{conv}"
                conv_t = all_tickers.get(conv_sym)
                conv_px = market_price_from_ticker(conv_t)
                if conv_px:
                    return float(raw) * float(conv_px)
        if qvol:
            for conv in ["USDT", "USDC", "USD"]:
                conv_sym = f"{q_upper}/{conv}"
                conv_t = all_tickers.get(conv_sym)
                conv_px = market_price_from_ticker(conv_t)
                if conv_px:
                    return float(qvol) * float(conv_px)
        return 0.0
    except:
        return 0.0

def symbol_ok(ex_obj, symbol):
    try:
        m = ex_obj.markets.get(symbol, {})
        # FIX #8: Default changed from True to False.
        # If the "spot" key is missing from market data, we should NOT assume
        # it is a spot market — that lets derivatives/perpetuals slip through.
        if not m or not m.get("spot", False): return False
        base, quote = parse_symbol(symbol)
        if not base or not quote: return False
        if quote.upper() not in USD_QUOTES: return False
        if LEV_REGEX.search(symbol): return False
        if m.get("active") is False: return False
        return True
    except:
        return False

def choose_common_chain(ex1, ex2, coin, exclude_chains, include_all_chains):
    try:
        c1 = ex1.currencies.get(coin, {}) or {}
        c2 = ex2.currencies.get(coin, {}) or {}
        nets1_raw = c1.get("networks", {}) or {}
        nets2_raw = c2.get("networks", {}) or {}

        # FIX #2: Network auth wall fix.
        # Binance, Bybit, MEXC, CoinEx and others require API keys to populate
        # the "networks" field inside currencies. Without keys, nets_raw is always
        # {}, making common_norm always empty, which previously returned "❌ No chain"
        # and blocked EVERY pair for those exchanges — zero results always.
        #
        # Fix: when either exchange has no network data, we cannot confirm OR deny
        # a common chain. Return "⚠️ Unverified" instead of a hard failure so the
        # opportunity is still surfaced with a warning rather than silently dropped.
        # The chain verification logic below still runs fully when data IS available.
        if not nets1_raw or not nets2_raw:
            return "⚠️ Unverified", "⚠️", "⚠️"

        # FIX #1: normalize_chain now maps to canonical names so aliases match:
        # BSC and BEP20 both → "BSC", MATIC and Polygon both → "POLYGON", etc.
        nets1_norm = {normalize_chain(k): (k, v) for k, v in nets1_raw.items()}
        nets2_norm = {normalize_chain(k): (k, v) for k, v in nets2_raw.items()}

        common_norm = set(nets1_norm.keys()) & set(nets2_norm.keys())
        if not common_norm:
            return "❌ No chain", "❌", "❌"

        exclude_norm = {normalize_chain(c) for c in exclude_chains} if not include_all_chains else set()
        preferred_norm = [normalize_chain(n) for n in LOW_FEE_CHAIN_PRIORITY if normalize_chain(n) not in exclude_norm]

        best_norm = next((p for p in preferred_norm if p in common_norm), None)
        if not best_norm:
            candidates = [c for c in common_norm if c not in exclude_norm]
            if not candidates: return "❌ No chain", "❌", "❌"
            best_norm = sorted(candidates)[0]

        orig_key1, info1 = nets1_norm[best_norm]
        orig_key2, info2 = nets2_norm[best_norm]
        w_ok = "✅" if info1.get("withdraw") else "❌"
        d_ok = "✅" if info2.get("deposit") else "❌"
        return best_norm, w_ok, d_ok
    except:
        return "❌ Unknown", "❌", "❌"

def fetch_tickers_safe(ex, name, logger=None):
    for attempt in range(3):
        try:
            return ex.fetch_tickers()
        except Exception as e:
            if attempt == 2:
                if logger:
                    logger(f"⚠️ fetch_tickers failed for {name}: {str(e)[:80]}")
                return {}
            time.sleep((2 ** attempt) * 1.5)
    return {}

# ====================== EXCHANGE CACHE (within one scan) ======================
# Shared across combo workers so the same exchange is not loaded/fetched
# multiple times if it appears in several combos (e.g. Binance paired with
# KuCoin, Bybit, Gate.io all in the same scan).
# Protected by its own lock since 3 combo threads run concurrently.

class ExchangeCache:
    def __init__(self):
        self._lock = threading.Lock()
        self._ex_objs = {}    # ex_id → ccxt exchange object
        self._tickers = {}    # ex_id → tickers dict
        self._failed = set()  # ex_ids that failed to load

    def get_exchange(self, ex_id, logger):
        with self._lock:
            if ex_id in self._failed:
                return None
            if ex_id in self._ex_objs:
                return self._ex_objs[ex_id]

        # Load outside lock so other threads aren't blocked during HTTP call
        try:
            opts = {"enableRateLimit": True, "timeout": 30000}
            opts.update(EXTRA_OPTS.get(ex_id, {}))
            ex = getattr(ccxt, ex_id)(opts)
            ex.load_markets()
            with self._lock:
                self._ex_objs[ex_id] = ex
            logger(f"✓ Loaded {EXCHANGE_NAMES.get(ex_id, ex_id)} ({len(ex.markets)} markets)")
            return ex
        except Exception as e:
            logger(f"⚠️ Skipped {EXCHANGE_NAMES.get(ex_id, ex_id)}: {str(e)[:80]}")
            with self._lock:
                self._failed.add(ex_id)
            return None

    def get_tickers(self, ex_id, logger):
        with self._lock:
            if ex_id in self._tickers:
                return self._tickers[ex_id]
            if ex_id in self._failed:
                return {}
            ex = self._ex_objs.get(ex_id)

        if ex is None:
            return {}

        # Fetch outside lock
        name = EXCHANGE_NAMES.get(ex_id, ex_id)
        tickers = fetch_tickers_safe(ex, name, logger)
        with self._lock:
            self._tickers[ex_id] = tickers
        logger(f"✓ Fetched tickers → {name} ({len(tickers)} tickers)")
        return tickers

    def cleanup(self):
        """Explicitly clear everything to help GC reclaim memory after scan."""
        with self._lock:
            for ex in self._ex_objs.values():
                try:
                    if hasattr(ex, 'markets'):    ex.markets.clear()
                    if hasattr(ex, 'currencies'): ex.currencies.clear()
                except:
                    pass
            self._ex_objs.clear()
            self._tickers.clear()
            self._failed.clear()


# ====================== SINGLE COMBO PROCESSOR ======================
def process_combo(b_id, s_id, cache, settings, logger):
    """
    Process one (buy_exchange, sell_exchange) pair.
    Called from a thread pool — all exchange data comes from the shared cache.
    Returns a list of opportunity dicts for this combo.
    """
    min_p        = settings.get("min_profit", 1.0)
    max_p        = settings.get("max_profit", 20.0)
    min_vol      = settings.get("min_24h_vol_usd", 100000.0)
    exclude      = settings.get("exclude_chains", ["ETH"])
    include_all  = settings.get("include_all_chains", False)
    MAX_PAIRS    = 200

    b_ex = cache.get_exchange(b_id, logger)
    s_ex = cache.get_exchange(s_id, logger)
    if not b_ex or not s_ex:
        return []

    b_tk = cache.get_tickers(b_id, logger)
    s_tk = cache.get_tickers(s_id, logger)
    if not b_tk or not s_tk:
        return []

    common = set(b_ex.markets) & set(s_ex.markets)
    symbols = [s for s in common if symbol_ok(b_ex, s) and symbol_ok(s_ex, s)]

    if not symbols:
        return []

    # FIX #12 (volume cap ordering):
    # Sort by volume score BEFORE capping at MAX_PAIRS so we keep the
    # highest-volume candidates for profit evaluation. Note: this is still
    # a volume-first pre-filter — the real fix for missing profitable pairs
    # would be to raise MAX_PAIRS, but keeping at 200 to protect memory.
    def vol_score(sym):
        bt  = b_tk.get(sym)
        st_ = s_tk.get(sym)
        pb  = market_price_from_ticker(bt) or 0
        ps  = market_price_from_ticker(st_) or 0
        return safe_usd_volume(b_id, sym, bt, pb, b_tk) + safe_usd_volume(s_id, sym, st_, ps, s_tk)

    symbols.sort(key=vol_score, reverse=True)
    symbols = symbols[:MAX_PAIRS]

    combo_results = []
    current_keys  = []

    for sym in symbols:
        bt  = b_tk.get(sym)
        st_ = s_tk.get(sym)
        if not bt or not st_ or not is_ticker_fresh(bt) or not is_ticker_fresh(st_):
            continue

        bp = market_price_from_ticker(bt)
        sp = market_price_from_ticker(st_)
        if not bp or not sp:
            continue

        # FIX #10: Removed the dead `if bp == 0: continue` line that was
        # already unreachable because `if not bp` above catches zero.

        if abs(sp - bp) / bp > 0.5:
            continue

        b_fee = b_ex.markets.get(sym, {}).get("taker", 0.001)
        s_fee = s_ex.markets.get(sym, {}).get("taker", 0.001)

        spread = (sp - bp) / bp * 100
        profit = spread - (b_fee * 100 + s_fee * 100)
        if profit < min_p or profit > max_p:
            continue

        b_vol = safe_usd_volume(b_id, sym, bt, bp, b_tk)
        s_vol = safe_usd_volume(s_id, sym, st_, sp, s_tk)
        if b_vol < min_vol or s_vol < min_vol:
            continue

        base, quote = parse_symbol(sym)
        chain, w_ok, d_ok = choose_common_chain(b_ex, s_ex, base, exclude, include_all)

        # FIX #2: Only block confirmed "❌" failures.
        # "⚠️ Unverified" (no API key, network data unavailable) is allowed through
        # so exchanges like Binance/Bybit/MEXC/CoinEx still surface opportunities.
        # "❌ No chain" or "❌ Unknown" are confirmed failures → skip.
        if w_ok == "❌" or d_ok == "❌":
            continue

        if not include_all and chain.startswith("❌"):
            continue

        key = f"{sym}|{b_id}>{s_id}"
        current_keys.append(key)
        obs, exp = stability_and_expiry(key, profit)

        combo_results.append({
            "Pair":                 sym,
            "Quote":                quote,
            "Buy@":                 EXCHANGE_NAMES.get(b_id, b_id),
            "Buy Price":            round(bp, 10),
            "Sell@":                EXCHANGE_NAMES.get(s_id, s_id),
            "Sell Price":           round(sp, 10),
            "Spread %":             round(spread, 4),
            "Profit % After Fees":  round(profit, 4),
            "Buy Vol (24h)":        fmt_usd(b_vol),
            "Sell Vol (24h)":       fmt_usd(s_vol),
            "Withdraw?":            w_ok,
            "Deposit?":             d_ok,
            "Blockchain":           chain,
            "Stability":            obs,
            "Est. Expiry":          exp,
        })

    logger(f"✓ {EXCHANGE_NAMES.get(b_id,b_id)} → {EXCHANGE_NAMES.get(s_id,s_id)}: {len(combo_results)} opportunities")

    # FIX #11: Return current_keys alongside results so the caller can build
    # a complete current_keys list. Previously, skipped combos (same exchange,
    # failed load) never added to current_keys, making those opportunities
    # always appear "disappeared" and corrupting lifetime/expiry estimates.
    return combo_results, current_keys


# ====================== CORE SCAN ======================
def run_scan(settings, logger):
    buy_ids     = settings.get("buy_exchanges", [])
    sell_ids    = settings.get("sell_exchanges", [])

    logger("🚀 Starting scan (3 combos at a time)")
    logger(f"Buy: {buy_ids} | Sell: {sell_ids}")

    if not buy_ids or not sell_ids:
        logger("❌ Need at least one buy & sell exchange")
        return []

    # Build all (buy, sell) combos — skip same-exchange pairs
    combos = [
        (b, s) for b in buy_ids for s in sell_ids if b != s
    ]

    if not combos:
        logger("❌ No valid combos (all buy/sell exchanges are the same)")
        return []

    logger(f"📋 {len(combos)} combos to process")

    # Shared exchange cache — loaded once per exchange, reused across combos
    cache = ExchangeCache()

    all_results   = []
    all_keys      = []
    results_lock  = threading.Lock()

    # FIX #12 (architecture): Process 3 combos concurrently instead of 1.
    # Each thread pulls exchange objects and tickers from the shared cache,
    # so Binance is only loaded/fetched once even if it appears in 5 combos.
    # Peak memory = all exchange objects loaded (shared) + 3 combo workloads.
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = {
            executor.submit(process_combo, b, s, cache, settings, logger): (b, s)
            for b, s in combos
        }

        for future in as_completed(futures):
            b_id, s_id = futures[future]
            try:
                result = future.result()
                if result:
                    combo_results, combo_keys = result
                    with results_lock:
                        all_results.extend(combo_results)
                        all_keys.extend(combo_keys)
            except Exception as e:
                logger(f"⚠️ Combo {b_id}→{s_id} failed: {str(e)[:80]}")

    # FIX #11: update_lifetime_for_disappeared now gets the full set of
    # current_keys collected from all combos that actually ran, not a partial
    # list that missed skipped combos.
    update_lifetime_for_disappeared(all_keys)

    # Cleanup exchange objects and tickers to free memory
    cache.cleanup()
    gc.collect()

    # Sort by profit descending
    all_results.sort(key=lambda x: x["Profit % After Fees"], reverse=True)

    logger(f"✅ Scan complete — {len(all_results)} total opportunities")
    return all_results


# ====================== ROUTES ======================
@app.route('/')
def index():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    html_path = os.path.join(base_dir, 'frontend.html')
    with open(html_path, 'r') as f:
        html = f.read()
    return render_template_string(html, TOP_EXCHANGES=TOP_EXCHANGES, EXCHANGE_NAMES=EXCHANGE_NAMES)

@app.route('/api/scan', methods=['POST'])
def api_scan():
    settings = request.get_json() or {}
    logs = []

    def logger(msg):
        ts   = time.strftime("%H:%M:%S")
        line = f"[{ts}] {msg}"
        print(line)
        logs.append(line)

    results = run_scan(settings, logger)

    # FIX #6: Only save settings when the scan had valid inputs and returned results.
    # Previously, empty or malformed settings were always saved, overwriting
    # good saved settings from a previous successful scan.
    buy_ids  = settings.get("buy_exchanges", [])
    sell_ids = settings.get("sell_exchanges", [])
    if buy_ids and sell_ids and results is not None:
        save_settings(settings)

    return jsonify({"results": results, "logs": logs})

@app.route('/api/exchanges')
def get_exchanges():
    return jsonify({
        "exchanges": TOP_EXCHANGES,
        "names":     EXCHANGE_NAMES
    })

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port)
