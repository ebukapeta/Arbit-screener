...
# Binance was missing from the original — caused fetch_tickers() to silently
...
# Now: normalize_chain("BSC") == normalize_chain("BEP20") == "BSC"
CHAIN_CANONICAL = {
    "BEP20":    "BSC",      "BSC":      "BSC",
...
def run_scan(settings, logger):
    ...
    for ex_id in all_ex_ids:
        ...
        try:
            ...
            logger(f"Tickers failed {name}: {str(e)[:80]}")
            failed_ids.add(ex_id)
