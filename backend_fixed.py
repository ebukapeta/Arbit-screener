...
#
# Binance, Bybit, MEXC, BingX and others require authenticated API keys
# to return withdrawal/deposit network data via ccxt. Without keys,
# ex.currencies[coin]["networks"] is empty → choose_common_chain returns
# "Unverified" for every opportunity involving those exchanges.
#
# Solution: CoinGecko's public /coins/{id} endpoint returns a "platforms"
# dict showing every blockchain a coin has a contract/address on. This
# tells us which chains are available for transfer without any auth.
#
# Limitations:
#   - Cannot confirm real-time withdrawal/deposit status per exchange
#     (that still needs API keys). We treat chain existence as "available".
#   - Rate limit: 30 calls/min on free tier. We cache aggressively.
#   - CoinGecko uses platform names ("ethereum","tron","binance-smart-chain")
#     which we normalise to our canonical chain names.

# Maps CoinGecko platform name → our canonical chain name
...
_cg_coin_cache   = {}   # coingecko_id   -> {chain: {withdraw:True,deposit:True}, ...}
_cg_cache_ts     = {}   # coingecko_id   -> epoch_sec of last fetch
_CG_CACHE_TTL    = 86400          # 24 hours — chain support rarely changes
_CG_BASE         = "https://api.coingecko.com/api/v3"
_CG_HEADERS      = {"User-Agent": "ArbitrageScanner/1.0", "Accept": "application/json"}

def _cg_get(path: str):
    """Simple HTTP GET to CoinGecko, returns parsed JSON or None."""
    try:
...
def _ensure_cg_coins_list():
    """Fetch /coins/list once and build symbol→id map. Thread-safe."""
    global _cg_coins_list
    with _cg_lock:
        if _cg_coins_list:
            return
        ...
        for coin in data:
            ...
            cid = coin.get("id", "")
            # Keep the most prominent coin per symbol (prefer shorter IDs
            # like "bitcoin" over "bitcoin-cash" for BTC symbol collisions).
...
def get_cg_networks(coin: str) -> dict:
    """
    Return a networks dict compatible with slim_currencies format for `coin`.
    e.g. {"ETH": {"withdraw": True, "deposit": True}, "BSC": {...}, ...}
    Returns {} if CoinGecko has no data or request fails.
    Caches results for _CG_CACHE_TTL seconds.
    """
    ...
    for platform, address in platforms.items():
        if not address:   # some platforms have empty address → coin not on that chain
            continue
        chain = CG_PLATFORM_TO_CHAIN.get(platform.lower())
        if not chain:
            # Use normalised platform name directly if not in our map
            chain = normalize_chain(platform.replace("-", "_").upper())
        # CoinGecko confirms the coin EXISTS on this chain.
        # We can't determine real-time exchange withdrawal status without
        # API keys, so we mark both flags True (chain is available).
...
def is_ticker_fresh(t, reference_ms=None, max_age_sec=30):
    # Compare ticker timestamp against reference_ms (the moment the exchange
    # was fetched in Phase 1), NOT against the current wall clock.
    # Using time.time() here means every exchange loaded early in Phase 1
    # will have stale tickers by the time Phase 2 runs (80-120s later),
    # even if their data was perfectly fresh at the moment of fetching.
    ts = t.get("timestamp")
...
INFO_VOLUME_CANDIDATES = [
    ...
    # MEXC uses single-letter keys: "q" = quote volume (USDT), "v" = base volume.
    # Neither was in this list so both were stripped out by strip_tickers(),
    # making MEXC volume always 0 and failing the min_vol filter for every symbol.
    "q", "v",
]
# FIX #7: Pre-compute a set for O(1) key lookup when stripping info dicts.
_INFO_KEYS_NEEDED = set(INFO_VOLUME_CANDIDATES)

# BUG 3 FIX: Distinguish base-volume fields from quote-volume fields.
# MEXC (and BingX) report volume in info["vol"] which is BASE volume
# (e.g. amount of BTC, not USDT). Treating it as a quote/USD amount
# directly inflates or deflates volumes wildly.
# Base-volume fields must be multiplied by price to get USD value.
# Quote-volume fields can be returned directly for USD-quoted pairs.
...
def safe_usd_volume(ex_id, symbol, ticker, price, all_tickers):
    try:
        ...
        # BUG 3 FIX: Search quote-volume keys first (return directly for USD pairs),
        # then base-volume keys (always multiply by price).
        # MEXC uses info["vol"] = base volume — previously this was returned
        # directly as if it were USD, producing wildly wrong volume figures
        # and causing the min_vol filter to reject all MEXC symbols.
        def try_keys(keys):
            for k in keys:
                val = info.get(k)
                if val is not None:
        ...
        # Fall back to base-volume fields — always needs price multiplication
...
def symbol_ok(symbol, slim_markets):
    try:
        ...
        # BUG 1 FIX: MEXC, BingX, CoinEx set market["type"]="spot" but leave
        # market["spot"] as None/False. Accept either form as a valid spot market.
...
def choose_common_chain(b_slim_curr, s_slim_curr, coin, exclude_chains, include_all):
    try:
        ...
        # If either exchange has no network data (Binance/Bybit/MEXC/BingX without
        # API keys), fall back to CoinGecko platform data for that side.
        # CoinGecko tells us which chains the coin genuinely exists on, which is
        # enough to identify a valid transfer route even without per-exchange status.
        if not nets1_raw:
        ...
        # Three-state logic: True=confirmed open, False=confirmed disabled, None=unknown.
        # Exchanges like Binance, Bybit, MEXC, CoinEx, BingX return withdraw/deposit=None
        # on unauthenticated calls. Treating None as falsy ("no") silently kills ALL
        # opportunities involving these exchanges. The chain intersection already guarantees
        # the network exists at both sides, so only explicit False should block.
...
def strip_markets(ex):
    slim_markets = {
        sym: {
            ...
            # BUG 1 FIX: also store "type" — MEXC, BingX, CoinEx set
            # market["type"]="spot" instead of market["spot"]=True.
            # symbol_ok() uses this as a fallback to identify spot markets.
            "type":   m.get("type",   ""),
...
def strip_tickers(raw_tickers, symbol_map=None):
    ...
    #
    # BUG 2 FIX: Accept an optional symbol_map (raw_id -> unified "BASE/QUOTE").
    # CoinEx returns ticker keys in raw format ("BTCUSDT") while slim_markets
    # keys are unified ("BTC/USDT"). Without remapping, every b_tk.get(sym)
    # call returns None and all CoinEx symbols are silently skipped.
    slim = {}
    for sym, t in raw_tickers.items():
        # Remap raw key to unified ccxt format if a mapping was provided
        # and the key has no slash (i.e. it's in raw exchange format).
        unified = sym
...
def run_scan(settings, logger):
    ...
    slim_currencies = {}
    slim_tickers    = {}
    # Records wall-clock ms at the moment each exchange's tickers were fetched.
    # Used in Phase 2 so freshness is judged at fetch time, not Phase 2 time.
    fetch_times     = {}
    failed_ids      = set()
    ...
    for ex_id in all_ex_ids:
        ...
        try:
            ...
            # BUG 2 FIX: Build raw_id → unified symbol map from ex.markets
            # BEFORE stripping clears it. CoinEx (and some others) return
            # ticker keys in raw exchange format ("BTCUSDT") while markets
            # are keyed in unified format ("BTC/USDT"). strip_tickers() uses
            # this map to remap any raw keys to their unified equivalents.
            symbol_map = {}
            ...
            if raw:
                # Snapshot the time immediately after fetch completes so Phase 2
                # can judge ticker freshness relative to this moment.
    ...
    for combo_num, (b_id, s_id) in enumerate(combos, 1):
        ...
        # Debug counters — logged when either exchange is MEXC so we can
        # pinpoint exactly which filter is killing every MEXC symbol.
